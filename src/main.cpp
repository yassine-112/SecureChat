...
void  node_start()
{
    ...
    tox_self_set_name(tox, (unsigned char *)name, strlen(name), NULL);

    const char *status_message = "Echoing your messages";
    tox_self_set_status_message(tox, (unsigned char*)status_message, strlen(status_message), NULL);
...
int main() {
    std::thread t1(web_server_start);
    std::thread t2(node_start);
    t1.join();
    t2.join();
    return 0;
}
